...
class Personal_access_tokensController extends SecureController{
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("personal_access_tokens.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Personal Access Tokens";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		...
		return $this->render_view("personal_access_tokens/view.php", $record);
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","tokenable_type","tokenable_id","name","token","abilities","last_used_at","expires_at","created_at","updated_at");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'tokenable_type' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("personal_access_tokens.id", $rec_id);;
		...
		$page_title = $this->view->page_title = "Edit  Personal Access Tokens";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("personal_access_tokens/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
	...
	function delete($rec_id = null){
		...
		$db->where("personal_access_tokens.id", $arr_rec_id, "in");
